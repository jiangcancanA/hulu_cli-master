...
/**
 * @description 计算传入时间与当前时间的差值
 * @param {String} [end] - 传入时间
 * @example params('2022-12-29 12：11：00')
 * @return 1天后到期
 * @example params('2022-12-27 12：11：00')
 * @return 已到期1天
 * */
export function PassDate(start) {
  if (start.includes('-')) start = start.replace(/-/g, '/')
  const nowDate = new Date().getFullYear() + '/' + (new Date().getMonth() + 1) + '/' + new Date().getDate() + ' 00: 00: 00'
  const startStamp = new Date(start).getTime()
  const endStamp = new Date(nowDate).getTime()
  let result = ''
  const daySum = Math.abs(new Date(start).getTime() - new Date().getTime()) / (60 * 60 * 1000)
  if (daySum < 24) { // 一天内
    // 剩余
  ...
  if (startStamp > endStamp) {
    // 剩余
    const total = (startStamp - endStamp) / 1000
    const year = Math.floor(total / (60 * 60 * 24 * 365))
    if (year) result += `${year}年`
    const day = Math.floor((total - year * (60 * 60 * 24 * 365)) / (60 * 60 * 24))
    if (day) result += `${day}天`
    result = result + '后到期'
  } else {
    // 到期
    ...
    const year = Math.floor(total / (60 * 60 * 24 * 365))
    if (year) result += `${year}年`
    const day = Math.ceil((total - year * (60 * 60 * 24 * 365)) / (60 * 60 * 24))
    if (day) result += `${day}天`
    result = '已到期' + result
  }
  return result
